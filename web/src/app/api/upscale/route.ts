...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const backendResponse = await fetch(`${BACKEND_URL}/api/upscale`, {
      method: 'POST',
      headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        { error: 'Backend Error', message: errorText || 'Failed to upscale image.' },
    ...
    console.error('[API Proxy Upscale Error]:', error);
    return NextResponse.json(
      { error: 'Connection Error', message: 'Failed to connect to LumaForge upscale backend.' },
      { status: 502 }
    );
  }
}
