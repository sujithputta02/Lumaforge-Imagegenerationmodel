...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const backendResponse = await fetch(`${BACKEND_URL}/api/generate-img2img`, {
      method: 'POST',
      headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        { error: 'Backend Error', message: errorText || 'Failed to modify image.' },
    ...
    console.error('[API Proxy Img2Img Error]:', error);
    return NextResponse.json(
      { error: 'Connection Error', message: 'Failed to connect to LumaForge generation backend.' },
      { status: 502 }
    );
  }
}
